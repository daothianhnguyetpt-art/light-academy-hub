...
export function Header({
  ...
  return <>
      ...
      duration: 0.5
        ...
        <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden lg:flex items-center gap-1">
                  ...
                  <NavigationMenuItem>
                    ...
                    <NavigationMenuContent>
                      <ul className="grid gap-1 p-2 w-[200px]">
                        {funAcademyItems.map(item => <li key={item.href}>
                            <NavigationMenuLink asChild>
                              <Link to={item.href} className="block select-none rounded-md p-3 leading-none no-underline outline-none transition-colors hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground">
              ...
              {/* Main Links */}
              {mainNavLinks.map(link => <Link key={link.href} to={link.href} className="px-4 py-2 text-sm font-medium text-muted-foreground hover:text-foreground transition-colors rounded-md hover:bg-accent/50">
                  {t(`nav.${link.key}`)}
                </Link>)}
            </nav>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="hidden sm:flex items-center gap-2">
                {loading ? <div className="w-24 h-9 bg-muted animate-pulse rounded-md" /> : user ?
              ...
              <DropdownMenu>
                  ...
                  </DropdownMenu> : isWalletConnected && walletAddress ?
              ...
              <Button variant="gold" onClick={() => setIsAuthDialogOpen(true)} className="flex items-center gap-2">
              ...
              <div className="flex flex-col gap-1 pt-4">
                ...
                <Collapsible open={isFunAcademyOpen} onOpenChange={setIsFunAcademyOpen}>
                  <CollapsibleTrigger className="flex items-center justify-between w-full px-4 py-3 text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-accent/50 rounded-md transition-colors">
                    ...
                    <ChevronDown className={cn("w-4 h-4 transition-transform", isFunAcademyOpen && "rotate-180")} />
                  </CollapsibleTrigger>
                  <CollapsibleContent className="pl-4">
                    {funAcademyItems.map(item => <Link key={item.href} to={item.href} onClick={() => setIsMobileMenuOpen(false)} className="block px-4 py-2 text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-accent/50 rounded-md transition-colors">
                        {t(`nav.${item.key}`)}
                      </Link>)}
                  </CollapsibleContent>
                </Collapsible>
                ...
                {/* Main Links */}
                {mainNavLinks.map(link => <Link key={link.href} to={link.href} onClick={() => setIsMobileMenuOpen(false)} className="px-4 py-3 text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-accent/50 rounded-md transition-colors">
                    {t(`nav.${link.key}`)}
                  </Link>)}

                {/* Mobile Auth Button */}
                <div className="pt-3 mt-2 border-t border-border/50">
                  {loading ? <div className="h-10 bg-muted animate-pulse rounded-md" /> : user ? <>
                      <Link to="/profile" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-2 px-4 py-3 text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-accent/50 rounded-md transition-colors">
                        <User className="w-4 h-4" />
                        {t("common.myProfile")}
                      </Link>
                      {isAdmin && <Link to="/admin" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-2 px-4 py-3 text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-accent/50 rounded-md transition-colors">
      ...
      {/* Auth Dialog */}
      <AuthDialog open={isAuthDialogOpen} onOpenChange={setIsAuthDialogOpen} onWalletConnect={onConnectWallet} isConnectingWallet={isConnectingWallet} connectingWalletType={connectingWalletType} />
    </>;
}